...
# Run the app first: python3 app/main.py  (or via Docker, mapped to :8080)

set -e
BASE_URL="${BASE_URL:-http://localhost:8080}"

echo "== Generating tokens =="
ALPHA_TOKEN=$(python3 tests/generate_token.py alpha)
BETA_TOKEN=$(python3 tests/generate_token.py beta)
...
curl -s -H "Authorization: Bearer $ALPHA_TOKEN" "$BASE_URL/search?account=ACC-1001" | python3 -m json.tool
...
curl -s -X POST -H "Authorization: Bearer $ALPHA_TOKEN" -H "Content-Type: application/json" \
    ...
    "$BASE_URL/transfer" | python3 -m json.tool
